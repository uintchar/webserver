...
- [5. 进程间通信](#5-进程间通信)
  - [5.1. 信号](#51-信号)
    - [5.1.1. 信号的基本概念](#511-信号的基本概念)
    - [5.1.2. 信号类型](#512-信号类型)
    - [5.1.3. 信号发送函数](#513-信号发送函数)
      - [5.1.3.1. kill()](#5131-kill)
      - [5.1.3.2. raise()](#5132-raise)
      - [5.1.3.3. abort()](#5133-abort)
    - [5.1.4. 信号捕捉函数](#514-信号捕捉函数)
      - [5.1.4.1. signal()](#5141-signal)
      - [5.1.4.2. sigaction()](#5142-sigaction)
    - [5.1.5. 信号集和信号掩码](#515-信号集和信号掩码)
    - [5.1.6. 内核实现信号捕捉的过程](#516-内核实现信号捕捉的过程)
    - [5.1.7. 多线程中的信号处理机制](#517-多线程中的信号处理机制)
    - [5.1.8. 应用示例](#518-应用示例)
      - [5.1.8.1. 信号捕捉实现周期性定时器](#5181-信号捕捉实现周期性定时器)
      - [5.1.8.2. 使用SIGCHLD信号解决僵尸进程的问题](#5182-使用sigchld信号解决僵尸进程的问题)
      - [5.1.8.3. 信号集操作函数范例](#5183-信号集操作函数范例)
  - [5.2. 匿名管道](#52-匿名管道)
...
- 信号因某些事件而产生后，会于稍后被传递给某一进程，而进程也会采取某些措施来响应信号。在产生和到达期间，信号处于等待（pending）状态。
- 一旦（内核）接下来要调度该进程运行，等待信号会马上送达，或者如果进程正在运行，则会立即传递信号（例如，进程向自身发送信号）。
- 有时需要确保一段代码不为传递来的信号所中断。为了做到这一点，可以将信号添加到进程的信号掩码中 — 目前会阻塞该组信号的到达。如果所产生的信号属于阻塞之列，那么信号将保持等待状态，直至稍后对其解除阻塞（从信号掩码中移除）。进程可使用各种系统调用对其信号掩码添加和移除信号。
</br>
...
Linux 下查看系统定义的信号列表：`kill –l`，其中前 31 个信号为常规信号，其余为实时信号。查看信号的详细信息： `man 7 signal`
...
- 如果接收的信号当前遭到阻塞，那么该信号将保持等待状态，直至解除对其阻塞。
- 系统不会对标准信号进行排队处理，也就是说，将信号标记为等待状态（以及后续的传递）只会发生一次。
