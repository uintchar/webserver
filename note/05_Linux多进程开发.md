...
- [5. 进程间通信](#5-进程间通信)
  - [5.1. 信号](#51-信号)
    - [5.1.1. 信号的概念](#511-信号的概念)
    - [5.1.2. 信号的类型](#512-信号的类型)
    - [5.1.3. 信号的发送](#513-信号的发送)
      - [5.1.3.1. kill()](#5131-kill)
      - [5.1.3.2. raise()](#5132-raise)
      - [5.1.3.3. abort()](#5133-abort)
    - [5.1.4. 信号的等待](#514-信号的等待)
    - [5.1.5. 信号的捕捉](#515-信号的捕捉)
      - [5.1.5.1. signal()](#5151-signal)
      - [5.1.5.2. sigaction()](#5152-sigaction)
    - [5.1.6. 信号集和信号掩码](#516-信号集和信号掩码)
      - [5.1.6.1. sigprocmask()](#5161-sigprocmask)
      - [5.1.6.2. sigpending()](#5162-sigpending)
    - [5.1.7. 信号处理器](#517-信号处理器)
      - [5.1.7.1. 设计信号处理器函数](#5171-设计信号处理器函数)
        - [5.1.7.1.1. 可重入函数和非可重入函数](#51711-可重入函数和非可重入函数)
        - [5.1.7.1.2. 异步信号安全函数](#51712-异步信号安全函数)
        - [5.1.7.1.3. 全局变量和 sig_atomic_t 数据类型](#51713-全局变量和-sig_atomic_t-数据类型)
      - [5.1.7.2. 终止信号处理器函数的其他方法](#5172-终止信号处理器函数的其他方法)
      - [5.1.7.3. 在备选栈中处理信号：sigaltstack()](#5173-在备选栈中处理信号sigaltstack)
      - [5.1.7.4. SA_SIGINFO 标志](#5174-sa_siginfo-标志)
      - [5.1.7.5. 系统调用的中断和重启](#5175-系统调用的中断和重启)
    - [5.1.8. 信号的高级特性](#518-信号的高级特性)
      - [5.1.8.1. 核心转储文件](#5181-核心转储文件)
      - [5.1.8.2. 传递、处置及处理特殊情况](#5182-传递处置及处理特殊情况)
      - [5.1.8.3. 可中断和不可中断的进程睡眠状态](#5183-可中断和不可中断的进程睡眠状态)
      - [5.1.8.4. 硬件产生的信号](#5184-硬件产生的信号)
    ...
    - [5.1.10. 应用示例](#5110-应用示例)
      ...
      - [5.1.10.3. 信号集操作函数范例](#51103-信号集操作函数范例)
  - [5.2. 匿名管道](#52-匿名管道)
    - [5.2.1. 管道的特点](#521-管道的特点)
    - [5.2.2. 管道的数据结构](#522-管道的数据结构)
    - [5.2.3. 匿名管道的使用](#523-匿名管道的使用)
    - [5.2.4. 应用示例](#524-应用示例)
      - [5.2.4.1. 父进程向子进程发送消息](#5241-父进程向子进程发送消息)
      - [5.2.4.2. 实现 `ps aux | grep xxx` 父子进程间通信](#5242-实现-ps-aux--grep-xxx-父子进程间通信)
      - [5.2.4.3. 设置管道非阻塞](#5243-设置管道非阻塞)
  - [5.3. 有名管道 FIFO](#53-有名管道-fifo)
    - [5.3.1. 有名管道的特点](#531-有名管道的特点)
    - [5.3.2. 有名管道的使用](#532-有名管道的使用)
    - [5.3.3. 应用示例](#533-应用示例)
      - [5.3.3.1. 不同进程使用有名管道进行通信](#5331-不同进程使用有名管道进行通信)
  - [5.4. 内存映射](#54-内存映射)
    - [5.4.1. 内存映射相关系统调用](#541-内存映射相关系统调用)
    - [5.4.2. 应用示例](#542-应用示例)
  - [5.5. 共享内存](#55-共享内存)
    - [5.5.1. 共享内存的概念](#551-共享内存的概念)
    - [5.5.2. 共享内存的使用步骤](#552-共享内存的使用步骤)
    - [5.5.3. 共享内存操作函数](#553-共享内存操作函数)
    - [5.5.4. 应用示例](#554-应用示例)
  - [5.6. POSIX消息队列](#56-posix消息队列)
    - [5.6.1. POSIX消息队列概述](#561-posix消息队列概述)
    - [5.6.2. mq_open()](#562-mq_open)
    - [5.6.3. mq_close()](#563-mq_close)
    - [5.6.4. mq_unlink()](#564-mq_unlink)
    - [5.6.5. mq_send()](#565-mq_send)
    - [5.6.6. mq_receive()](#566-mq_receive)
    - [5.6.7. mq_notify()](#567-mq_notify)
    - [5.6.8. mq_attr](#568-mq_attr)
    - [5.6.9. POSIX消息队列限制](#569-posix消息队列限制)
...
#### 5.2.4.2. 实现 `ps aux | grep xxx` 父子进程间通信
